...
export class DataAccessService {
  ...
  async findMany<T>(model: EntityTarget<T>, options: any): Promise<T[]>
  async findMany<T>(model: EntityTarget<T>, options: any, dataSource: DataSource): Promise<T[]>
  async findMany<T>(model: EntityTarget<T>, options: any, dataSource?: DataSource): Promise<T[]> {
    dataSource = dataSource || MysqlDataSource

    return await dataSource.manager.find(model, {
      where: options?.where || {},
      order: options?.order,
      skip: options?.skip,
      take: options?.take
